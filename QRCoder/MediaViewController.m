...
- (NSArray *)listArray {
    if (!_listArray) {
        _listArray = @[kRecordVideoUseImagePickerController,kPlayVideoWithAVPlayerViewController,kVideoSaveToAlbum,kGetMediaInfoWithAVAsset,kConvertWithAVAssetExportSession,kCompressWithAVAssetExportSession,kVideoFrameWithAVAssetImageGenerator,kUploadVideoToServer,kSelectVideoOfPhotoLibrary];
    }
...
//    NSLog(@"%@ - %@", mediaType, movieType);
    ...
    if ([movieType isEqualToString:mediaType]) {
        ...
        if (seconds > kVideoMaximumDuration) {
            NSLog(@"over time.");
            return;
        }
